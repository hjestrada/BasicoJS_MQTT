#include <WiFi.h>
#include <PubSubClient.h>

const char* ssid = "Teranet_Estrada";
const char* password = "1117508100Abc*";

const char* mqtt_server = "3fb2c2fed242415d88ab467b8ad1f686.s1.eu.hivemq.cloud";
const int mqtt_port = 8883;
const char* mqtt_username = "Colmenas";
const char* mqtt_password = "123456789Ab";

// Definición de los canales a utilizar
const char* CONTROL_LED_TOPIC = "1117508100/7448C055B594/Datos";
const char* VALOR_ANALOGICO_TOPIC = "1117508100/7448C055B594/Datos";

const int ledPin = 26;
const int inputPin = 36;

static const char* root_ca PROGMEM = R"EOF(
-----BEGIN CERTIFICATE-----

-----END CERTIFICATE-----
)EOF";

WiFiClient espClient; // Cambiando a WiFiClient
PubSubClient client(espClient);

unsigned long lastMsg = 0;
char msg[50];

void setup_wifi() {
  delay(10);
  Serial.println();
  Serial.print("Connecting to ");
  Serial.println(ssid);
  WiFi.begin(ssid, password);
  while (WiFi.status() != WL_CONNECTED) {
    delay(500);
    Serial.print(".");
  }
  Serial.println("");
  Serial.println("WiFi connected");
  Serial.println("IP address: ");
  Serial.println(WiFi.localIP());
}

void callback(char* topic, byte* payload, unsigned int length) {
  Serial.print("Mensaje recibido en el canal: ");
  Serial.println(topic);

  if (strcmp(topic, CONTROL_LED_TOPIC) == 0) {
    int estado = payload[0] - '0';
    digitalWrite(ledPin, estado);
    Serial.print("Estado del LED: ");
    Serial.println(estado);
  } else if (strcmp(topic, VALOR_ANALOGICO_TOPIC) == 0) {
    int valor_analogico = analogRead(inputPin);
    Serial.println("Valor analógico: "+String(valor_analogico));
    client.publish(VALOR_ANALOGICO_TOPIC, String(valor_analogico).c_str(), true);
    delay(2000);
  }
}

void reconnect() {
  while (!client.connected()) {
    Serial.print("Attempting MQTT connection...");
    if (client.connect("ESP32Client", mqtt_username, mqtt_password)) {
      Serial.println("connected");
      client.subscribe(CONTROL_LED_TOPIC);
      client.subscribe(VALOR_ANALOGICO_TOPIC);
    } else {
      Serial.print("failed, rc=");
      Serial.print(client.state());
      Serial.println(" try again in 5 seconds");
      delay(5000);
    }
  }
}

void setup() {
  pinMode(ledPin, OUTPUT);
  pinMode(inputPin, INPUT); // Cambiado a INPUT
  Serial.begin(9600);
  setup_wifi();
  // Eliminado espClient.setCACert(root_ca);
  client.setServer(mqtt_server, mqtt_port);
  client.setCallback(callback);
}

void loop() {
  if (!client.connected()) {
    reconnect();
  }
  client.loop();
}
